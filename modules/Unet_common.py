...
def iwt_init1d(x):
    '''
    x:[B,C,H,W] 12 channels
    return:[B,C,H,W] 3 channels
    '''
    r = 2
...
def dwt_init(x):
    '''
    x:[B,C,H,W] 3 channels
    return:[B,C,H,W] 12 channels
    '''
    x01 = x[:, :, 0::2, :] / 2  # 取偶数行
    x02 = x[:, :, 1::2, :] / 2  # 取奇数行
    x1 = x01[:, :, :, 0::2]  # 偶数行的偶数列
    ...
    x3 = x01[:, :, :, 1::2]  # 偶数行的奇数列
    x4 = x02[:, :, :, 1::2]  # 奇数行的奇数列
    # x1和x2 x3和x4之间的操作是列变换，x1x2和x3x4之间的操作是行变换
    x_LL = x1 + x2 + x3 + x4
    x_HL = -x1 - x2 + x3 + x4  # 写反了 但是和恢复的程序对应应该就可以吧
    x_LH = -x1 + x2 - x3 + x4  # 写反了
    x_HH = x1 - x2 - x3 + x4
...
def iwt_init(x):
    ...
    x1 = x[:, 0:out_channel, :, :] / 2  # LL
    x2 = x[:, out_channel:out_channel * 2, :, :] / 2  # HL
    x3 = x[:, out_channel * 2:out_channel * 3, :, :] / 2  # LH
    x4 = x[:, out_channel * 3:out_channel * 4, :, :] / 2  # HH
